function [concentrationMatrix, excRxnNames, timeVec, biomassVec,mu] = dynamicFBAm(model, substrateRxns, initConcentrations, initBiomass, timeStep, nSteps, plotRxns, exclUptakeRxns)
% Performs dynamic FBA simulation using the static optimization approach
%-----Modified to allow for forced production of peptides and folic acid
%from ST and LB
%
% USAGE:
%
...
if ~isempty(intersect(model.rxns(find(model.lb>0)),excRxnNames))
    %uptakeBound(getPosOfElementsInArray(model.rxns(find(model.lb>0)), excRxnNames)) = makeItWork(model.rxns(find(model.lb>0)), excRxnNames ,model.lb(find(model.lb>0)))
    rxns=intersect(model.rxns(find(model.lb>0)),excRxnNames);
    pos_in_model=getPosOfElementsInArray(rxns,model.rxns);
    uptakeBound(getPosOfElementsInArray(rxns, excRxnNames)) = -model.lb(pos_in_model);
end
   % uptakeBound(getPosOfElementsInArray(model.rxns(find(model.lb>0)), excRxnNames)) = -model.lb(find(model.lb>0)) ;
...
selNonZero = any(concentrationMatrix>=0,2);%!!!get rid of equal sign
...
%title('Biomass');
%subplot(1,2,2);
%lot(timeVec,concentrationMatrix(selPlot,:));
%xis tight
